...
> **نسخه سند:** 1.5 · **آخرین به‌روزرسانی:** 2026-09-24  
...
1. [معرفی و دو حالت استقرار](#1-معرفی-و-دو-حالت-استقرار)
...
8. [لایسنس Enterprise](#8-لایسنس-enterprise)
...
## 1. معرفی و دو حالت استقرار

Morobot یک سامانه اتوماسیون فرآیندهای وب است که روی SQL Server اجرا می‌شود و با افزونه مرورگر Chrome کار می‌کند.

| حالت | توضیح |
|------|--------|
| **Cloud** | سرویس ابری — مدیریت پلن‌ها و سقف‌ها از داخل پنل مدیریت |
| **Enterprise (on-prem)** | نصب روی سرور سازمان — نیاز به لایسنس رسمی دارد |

برای نصب روی سرور سازمان، فایل `appsettings.json` را مطابق راهنمای بخش ۵ تنظیم کنید. مقدار `Morobot:DeploymentMode` باید `Enterprise` باشد. برای هر استقرار جدا (Cloud چند دامنه، Enterprise، dev) **`Morobot:AppInstanceKey`** یکتا تعریف کنید.
...
3. **داده‌های موجود (کاربر، فرآیند، منبع) حذف نمی‌شوند** — فقط اطلاعات لایسنس به‌روز می‌شود
...
- از **اولین اجرا** در حالت Enterprise شروع می‌شود
