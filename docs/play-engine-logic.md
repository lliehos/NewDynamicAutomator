...
هر شرط دو شاخهٔ منطقی دارد و **هر دو باید سیم‌کشی شوند** (مگر اینکه شرط را به‌عنوان یک
اقدام ترتیبی ساده استفاده کنید):

| وضعیت شرط | یالی که دنبال می‌شود |
|-----------|----------------------|
| نتیجه `true` | فقط یال `success` |
| نتیجه `false` | فقط یال `fail` |
| شاخهٔ موردنیاز **وصل نیست** | **توقف** + پیام «شاخهٔ «X» این شرط وصل نشده …» |
| شرط **هیچ** یال `success`/`fail` ندارد | یال `next` (اقدام ترتیبی) |

> **قاعدهٔ قدیمی (حذف‌شده):** اگر شاخهٔ موردنیاز نبود، موتور روی یال `next` می‌افتاد.
> نتیجه: یک شرطِ ناموفق می‌توانست شاخهٔ موفق را اجرا کند. این ایراد جدی بود و برطرف شد.

**اعتبارسنجی پیش از اجرا:** هم ویرایشگر و هم موتور، شرطی که یکی از شاخه‌هایش وصل نیست را
پیش از شروع اجرا علامت‌دار می‌کنند، پس نیازی نیست تا میان اجرا منتظر ماند.
...
4. ✅ **برطرف شد — سقوط شرط به یال `next` حذف شد.**
   پیش‌تر: `flowEdge(edges, cur, pass ? ["success","next"] : ["fail","next"])`.
   اگر شاخهٔ موردنیاز وصل نبود، موتور می‌افتاد روی یال `next` — یعنی یک شرطِ ناموفق
   می‌توانست شاخهٔ **موفق** را اجرا کند.
   اکنون:
   - اگر شرط **هرگونه** یال `success`/`fail` داشته باشد، فقط همان شاخهٔ لازم دنبال می‌شود.
   - اگر آن شاخه وصل نباشد → **توقف** با پیام روشن که نام شاخهٔ غایب را می‌گوید
     (`run.condMissingBranch`).
   - اگر شرط **هیچ** یال شاخه‌ای نداشته باشد، همچنان یال `next` به‌عنوان اقدام ترتیبی
     معتبر است.
   - همان قاعده در **اعتبارسنجی قبل از اجرا** هم اعمال می‌شود
     (`validateConditionNodeForPlay` و `validateConditionNode` در ویرایشگر)، پس
     ایراد پیش از شروع اجرا دیده می‌شود، نه در میان اجرا.
   رفتار مشاهده‌شده پس از اصلاح: `شرط(false) + fail وصل + next اضافی` →
   مسیر `next` **اجرا نمی‌شود**.
