...
> **نسخه سند:** 1.0 · **آخرین به‌روزرسانی:** 2026-09-25
...
**دو محدودیت ساختاری مهم:**

1. **`visited` Set:** اگر `cur` قبلاً دیده شده باشد →
   هشدار «توقف به‌خاطر حلقهٔ تکراری در نود …» و **`break`**.
   یعنی هر نود در یک پیمایش **فقط یک بار** اجرا می‌شود؛ **join الماسی** و **بازگشت به نود قبلی**
   به‌عنوان «حلقهٔ تکراری» تلقی و کل اجرا قطع می‌شود.
2. **`guard < 500`:** گراف بلندتر از ۵۰۰ گره **بی‌صدا** تمام می‌شود (هیچ خطایی داده نمی‌شود).
...
2. **`visited` در `executeFlow` (خط ۱۴۷۹):** هر نود در یک پیمایش **فقط یک بار** اجرا می‌شود.
   join الماسی و هر بازگشت به نود قبلی → «توقف به‌خاطر حلقهٔ تکراری» و **قطع کل اجرا**.
3. **`guard < 500` (خط ۱۴۷۵):** گراف بلندتر از ۵۰۰ گره **بی‌صدا** پایان می‌یابد (بدون خطا/هشدار).
